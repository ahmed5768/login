...
var emptyArr = []
  for(var i = 0; i<items.length ; i++){
    ...
    div1.className = "login-box"
    div1.innerHTML = `<h1>${items[i].title}</h1> <p><b>Description: </b>${items[i].description}</p> 
    <h4><b>Rating: </b>${items[i].rating}</h4> <h5><b>Stock: </b>${items[i].stock}</h5>`
    emptyDiv.appendChild(div1)
    emptyArr.push(items[i].category)
  }
var category = []
for(var a = 0 ; a < emptyArr.length ; a++){
    if(!category.includes(emptyArr[a])){
        category.push(emptyArr[a])
    }
}
var dropDown = document.getElementById("dropDown")
for(var b = 0 ; b < category.length; b++){
    dropDown.innerHTML += `<option value="${category[b]}">${category[b]}y</option>`
}
var brand = document.getElementById("brand")
dropDown.addEventListener("click", function(){
    var selectCatagory = dropDown.value 
    brand.innerHTML = ` <option value="">Brand</option>`
    for (let c = 0; c < items.length; c++) {
        if(selectCatagory == items[c].category){
            brand.innerHTML +=  `<option value="${items[c].brand}">${items[c].brand}y</option>`
        }       
    }
...
brand.addEventListener("click", function(){
    section.style.height = 100+"vh"
    var selectBrand = brand.value
    emptyDiv.innerHTML = ""
    brandDiv.innerHTML = ""
    for(var d = 0 ; d < items.length ; d++){
        if(selectBrand == items[d].brand){
            var brandCard = document.createElement("div")
            brandCard.className = "login-box"
            brandCard.innerHTML = `<h1>${items[d].title}</h1> <p><b>Brand: </b>${items[d].brand}<p><h4><b>Price: </b>${items[d].price}</h4> <h5><b>Discount Percentage: </b>${items[d].discountPercentage}</h5>` 
            brandDiv.appendChild(brandCard)
        }
    }
